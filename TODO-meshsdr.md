...
- ✅ Sync-Wort (0x2B) -> Symbol-Übersetzung: Byte→Symbol-Abbildung ist per Ground-Truth-FFT an zwei unabhängigen Werten (0x12 und 0x34) empirisch geklärt (Nibble × `1 << (sf-8)`). Hinweis: Skalierungsformel nur bei SF11 getestet.
...
* **Status:** ⚠️ OPEN (2026-08-24)
* **Description:** Payload-Symbol-Vergleich gegen angenommene Gray-Ground-Truth schlägt fehl. RX-Dechirp selbst ist per Energiekonzentrationstest nachweislich exakt (peak_energy_frac=1.0 auf allen getesteten Symbolen). Der Fehler liegt vermutlich in der Herleitung der Vergleichswerte (zuvor angenommene Werte wie 1297, 369, 1985), nicht im Dechirp, da die extrahierten Rohsymbole exakt mit der tatsächlichen `gr-lora_sdr` Ausgabe übereinstimmen.
* **Next Steps:** Die Payload-Dekodierung (`decode_symbols_to_payload`) und CRC-Prüfung muss anhand der korrekten Rohsymbole weiter debugged werden.
